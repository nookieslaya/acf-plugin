...
$repository = new class( $snapshot ) implements \AcfSchemaGuard\Snapshots\SnapshotRepository { private $snapshot; public function __construct( $snapshot ) { $this->snapshot = $snapshot; } public function insert( \AcfSchemaGuard\Snapshots\SchemaSnapshot $snapshot ) {} public function find( $id ) { return $this->snapshot->id() === $id ? $this->snapshot : null; } public function latest_for_source( $source_id ) { return null; } public function latest() { return $this->snapshot; } public function recent( $limit ) { return array( $this->snapshot ); } public function all() { return array( $this->snapshot ); } };
