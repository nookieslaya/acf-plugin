...
class AcfSchemaGuardDiffRepository implements \AcfSchemaGuard\Snapshots\SnapshotRepository { private $items; public function __construct($items){$this->items=$items;} public function insert(\AcfSchemaGuard\Snapshots\SchemaSnapshot $s){} public function find($id){return isset($this->items[$id])?$this->items[$id]:null;} public function latest_for_source($id){return null;} public function latest(){return null;} public function recent($limit){return array_slice(array_values($this->items),0,$limit);} public function all(){return array_values($this->items);} }
