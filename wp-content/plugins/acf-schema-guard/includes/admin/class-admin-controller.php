...
final class AdminController {
	...
	private function render_settings_page( array $screen ) {
		...
		<div class="wrap acf-schema-guard-admin acf-schema-guard-settings-page">
			...
			<section class="acf-schema-guard-settings-card">
				...
				<?php if ( \AcfSchemaGuard\Plugin::instance()->risk_policy()->has_team_policy() ) : ?><p class="acf-schema-guard-team-policy-notice"><strong><?php echo esc_html__( 'Team policy active', 'acf-schema-guard' ); ?></strong><br /><?php echo esc_html( sprintf( __( 'The repository file %1$s sets the active %2$s threshold. It takes priority over this site\'s local setting.', 'acf-schema-guard' ), 'acf-schema-guard-policy.json', ucfirst( $policy->fail_on() ) ) ); ?></p><?php endif; ?>
	...
	private function render_unused_fields_page( array $screen ) {
		...
		?>
		<div class="notice notice-info inline acf-schema-guard-unused-fields-development"><p><?php echo esc_html__( 'This workspace is in development. It complements Code Usage by starting with ACF fields that have no detected literal PHP reference. Review dynamic calls, unscanned code, and stored data before making any schema decision.', 'acf-schema-guard' ); ?></p></div>
