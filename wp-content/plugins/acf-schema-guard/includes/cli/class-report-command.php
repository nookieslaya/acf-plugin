...
final class ReportCommand {
	private $scanner;
	public function __construct( CodeUsageScannerService $scanner ) { $this->scanner = $scanner; }
	public function export( $args, $assoc_args ) {
		...
		$items = array(); foreach ( $this->scanner->scan( ( new ScannerConfiguration() )->roots() ) as $reference ) { $items[] = $reference->to_array(); }
	...
	private function markdown( array $items ) { $lines = array( '# ACF code usage report', '', '| Field | File | Line | Expression |', '| --- | --- | ---: | --- |' ); foreach ( $items as $item ) { $lines[] = sprintf( '| `%s` | `%s` | %d | `%s` |', $item['field_name'], $item['path'], $item['line'], str_replace( '|', '\\|', $item['expression'] ) ); } return implode( "\n", $lines ) . "\n"; }
}
