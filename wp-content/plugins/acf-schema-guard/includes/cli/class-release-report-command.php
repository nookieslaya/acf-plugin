...
final class ReleaseReportCommand {
	...
	public function export( $args, $assoc_args ) {
		if ( ! $this->capabilities->can( ProCapabilities::REVIEW_READY_REPORTS )->is_allowed() ) { \WP_CLI::error( 'Review-ready reports are available in Pro. Existing code-usage export remains available in Free.' ); }
		$path = isset( $args[0] ) ? (string) $args[0] : '';
		$format = isset( $assoc_args['format'] ) ? (string) $assoc_args['format'] : 'markdown';
		if ( '' === $path || ! in_array( $format, array( 'markdown', 'json' ), true ) ) { \WP_CLI::error( 'Provide an output path and --format=markdown or --format=json.' ); }
		if ( file_exists( $path ) && empty( $assoc_args['force'] ) ) { \WP_CLI::error( 'Report already exists. Use --force to replace it.' ); }
		$live = call_user_func( $this->analysis ); if ( ! $live->is_available() ) { \WP_CLI::error( $live->message() ); }
		$data = $live->analysis()->to_array(); foreach ( $data['findings'] as &$finding ) { $exception = $this->exceptions ? $this->exceptions->active_for( $finding ) : null; $finding['approved_exception'] = $exception ? $exception->to_array() : null; } unset( $finding );
		$report = array( 'schema_version' => 1, 'policy' => $this->policy ? $this->policy->policy()->fail_on() : 'high', 'findings' => $data['findings'] );
		$content = 'json' === $format ? wp_json_encode( $report, JSON_PRETTY_PRINT ) : $this->markdown( $report, $assoc_args );
		if ( false === file_put_contents( $path, $content ) ) { \WP_CLI::error( 'Could not write release report.' ); }
		\WP_CLI::success( 'Release report exported: ' . $path );
	}
